...
import pandas as pd
import torch
from sklearn.metrics import roc_auc_score, jaccard_score, accuracy_score, precision_recall_fscore_support
import cv2
...
class WeightedDiceLoss(nn.Module):
    ...
    def forward(self, logit, truth, smooth=1e-5):
        ...
        logit = logit.view(batch_size, -1) #[2,50176]
        truth = truth.view(batch_size, -1) #[2,50176]
        assert (logit.shape == truth.shape)
...
class CosineSimilarityLoss(nn.Module):
    ...
    def forward(self, img, text, label):
        cos_sim = torch.cosine_similarity(img, text)
...
def acc_on_batch(masks, pred):
    ...
    for i in range(pred.shape[0]):
        ...
        mask_tmp[mask_tmp > 0] = 1
        mask_tmp[mask_tmp <= 0] = 0
        accs.append(accuracy_score(mask_tmp.reshape(-1), pred_tmp.reshape(-1)))
    return np.mean(accs)
