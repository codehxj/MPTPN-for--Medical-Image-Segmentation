...
class PixLevelModule(nn.Module):
    def __init__(self, in_channels):
        ...
        #self.conv = nn.Conv2d(in_channels, out_channels=in_channels // 3, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        #x_ = self.conv(x)
        x_avg = self.conv_avg(x) #[2,512,28,28] 不变
        x_avg = self.relu_avg(x_avg) 
        x_avg = torch.mean(x_avg, dim=1)
        x_avg = x_avg.unsqueeze(dim=1) #[2,1,28,28]
        x_max = self.conv_max(x)
        x_max = self.relu_max(x_max)
        x_max = torch.max(x_max, dim=1).values #[2,28,28]
        x_max = x_max.unsqueeze(dim=1) #[2,1,28,28]
        x_out = x_max+x_avg
        x_output = torch.cat((x_avg, x_max, x_out), dim=1) #[2,3,28,28]
        x_output = x_output.transpose(1, 3)  #[2,28,28,3]
        x_output = self.bottleneck(x_output) #[2,28,28,1]
        x_output = x_output.transpose(1, 3)  #[2,1,28,28]
        y = x_output * x #[2,512,28,28]
        return y
