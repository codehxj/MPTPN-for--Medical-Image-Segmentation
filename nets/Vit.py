...
class Reconstruct(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)
        x = x.contiguous().view(B, hidden, h, w)  #[2,64,14,14]
        x = nn.Upsample(scale_factor=self.scale_factor)(x) #[2,64,224,224] [2,128,112,112] [2,256,56,56] [2,512,28,28]
...
class VisionTransformer(nn.Module):  # Transformer-branch
    def __init__(self, config, vis, img_size, channel_num, patch_size, embed_dim, depth=1, num_heads=8,
        ...
        self.CTBN = ConvTransBN(in_channels=embed_dim, out_channels=embed_dim//2)  #(64, 32)
        self.CTBN2 = ConvTransBN(in_channels=embed_dim*2, out_channels=embed_dim)  #(128, 64)
        self.CTBN3 = ConvTransBN(in_channels=10, out_channels=196)   #!!!!!!!!!!!out=196

    def forward(self, x, skip_x, text, reconstruct=False): #x[2,64,224,224]
        if not reconstruct:
            x = self.embeddings(x) #x[2,196,64]
            if self.dim == 64:
                x = x+self.CTBN3(text)  # [B, num_patches, embed_dim]=[2,196,64] conv+BN+ReLU
            x = self.Encoder_blocks(x)  #x[2,196,64] drop_path+attention+mlp
        else:
            x = self.Encoder_blocks(x)
        if (self.dim == 64 and not reconstruct) or (self.dim == 256 and reconstruct):
            return x
