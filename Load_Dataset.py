...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        image, label, text, text_clip = sample['image'], sample['label'], sample['text'], sample['text_clip']
        ...
        sample = {'image': image, 'label': label, 'text': text, 'text_clip': text_clip}
        return sample
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image_filename = self.images_list[idx]  # MoNuSeg Glas
        # mask_filename = image_filename[: -3] + "png"  # MoNuSeg Glas
        mask_filename = self.mask_list[idx]  # Covid19
        image_filename = mask_filename.replace('mask_', '')  # Covid19
        image = cv2.imread(os.path.join(self.input_path, image_filename))
        image = cv2.resize(image, (self.image_size, self.image_size)) #(224,224,3)
